...
一、锁
说到锁，就要和咱们的存储相挂钩
在MyIsam中，存在两种锁，共享读锁与独占写锁
在Innodb中，这两种锁，则叫共享锁与排他锁

Innodb中：
加排他锁：for update;
加共享锁：Lock in share model

（1）锁的基本介绍
>![avatar](/Users/liufuwei/Documents/my-project/my-juc/JUC/myJuc/image/锁的基本介绍.png)


（2）MyIsam的表锁
>![avatar](/Users/liufuwei/Documents/my-project/my-juc/JUC/myJuc/image/MyIsam的表锁.png)

myisam写阻塞读的操作
>![avatar](/Users/liufuwei/Documents/my-project/my-juc/JUC/myJuc/image/myisam写阻塞读的操作.png)

独占写锁，当前会话正在写这个表时，其他会话要对这个表的操作时，读写均不能进行
一定要注意，独占与排他是不一样的，后面会说到，这也就是为什么两种存储引擎，对锁的称呼是不一样的原因，这里面肯定是有一定道理的


MyIsam共享读锁
>![avatar](/Users/liufuwei/Documents/my-project/my-juc/JUC/myJuc/image/MyIsam共享读锁.png)

共享读锁，当前会话和其他会话都能对数据进行读取，但是都无法对数据进行更新，也不能对表进行添加数据的操作
另外，对表加了共享读锁之后，不仅不能对表进行更新操作外，还不能对其他表进行读写操作
...
|                           session1                           |                           session2                           |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| set autocommit=0~<br />~select \* from tab\_no\_index where id = 1; | set autocommit=0~<br />~select \* from tab\_no\_index where id =2 |
...
|                           session1                           |                           session2                           |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| set autocommit=0~<br />~select \* from tab\_with\_indexwhere id = 1; | set autocommit=0~<br />~select \* from tab\_with\_indexwhere id =2 |
|     select \* from tab\_with\_indexwhere id = 1 for update      |                                                              |
|                                                              |     select \* from tab\_with\_indexwhere id = 2 for update;     |
...
|                                                              | select \* from tab\_with\_index where id = 1 and name='4' for update~<br />~虽然session2访问的是和session1不同的记录，但是因为使用了相同的索引，所以需要等待锁 |

### 总结

**对于MyISAM的表锁，主要讨论了以下几点：** 
（1）共享读锁（S）之间是兼容的，但共享读锁（S）与排他写锁（X）之间，以及排他写锁（X）之间是互斥的，也就是说读和写是串行的。 （2）在一定条件下，MyISAM允许查询和插入并发执行，我们可以利用这一点来解决应用中对同一表查询和插入的锁争用问题。 
（3）MyISAM默认的锁调度机制是写优先，这并不一定适合所有应用，用户可以通过设置LOW\_PRIORITY\_UPDATES参数，或在INSERT、UPDATE、DELETE语句中指定LOW\_PRIORITY选项来调节读写锁的争用。 
